...
function loadBasemap(){
	...
	for(var i =0;i < gltfModel.length; i++){
		scene.primitives.remove(gltfModel[i]);
		gltfModel = [];
	}
	viewAddBottomMap(); 
	setEntitiesModel(ModelCenters[floorid]);
	setEntitiesBackground(shapeBackgrounds[floorid]);
	// if(gltfModel.length==0){console.log(666);setEntitiesPolygon(shapePolygons[floorid],shapePenups[floorid]);}
